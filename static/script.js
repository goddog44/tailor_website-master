function validateLoginForm() {
  ...
  if (!validatePassword(password.value)) {
    setError(
      ...
      "Password must be at least 6 characters long and contain at least 1 uppercase letter, 1 lowercase letter, and 1 special character."
    );
    valid = false;
  }
...
function validateSignupForm() {
  ...
  const password = document.querySelector(
    '.registration input[name="password1"]'
  );
  const confirmPassword = document.querySelector(
    '.registration input[name="password2"]'
  );
  let valid = true;
